...
df_hour = pd.read_csv('E:/KULIAH/Semester 6 (Bangkit)/Code/proyek_analisis_dicoding/hour.csv')
df_day = pd.read_csv('E:/KULIAH/Semester 6 (Bangkit)/Code/proyek_analisis_dicoding/day.csv')
#st.write(df)
...
with hourly_container:
    ...
    sns.lineplot(x="hr", y="cnt", data=df_hour, ci=None, ax=ax_hourly)
...
# korelasi cuaca
st.subheader('Korelasi cuaca dengan jumlah rental sepeda')
weather_container = st.container()
with weather_container:
    fig_weather, ax_weather = plt.subplots(figsize=(10, 6))
    sns.barplot(x="weathersit", y="cnt", data=df_day, palette="Set3", ax=ax_weather)
    ax_weather.set_title("Korelasi cuaca dengan rental")
